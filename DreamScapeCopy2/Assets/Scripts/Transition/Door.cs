...
public class Door : Interactable
{
    ...
    void Start() //загружаем все данные о персонаже, двери и уравнении
    {
        ...
        LoadDoorState();
        //AchievementManager.Instance.achievements.LoadAchievement();
        //needsKey = true;
        //Debug.Log(doorName);
        ...
        if (GlobalControl.Instance.IsSceneBeingLoaded) //если мы загружаем  персонажа то передаем значения по XYZ
        {
    ...
    void Update()
    {
        ...
        foreach (ItemInteract item in items) // проверка на наличие серого ключа в инвентаре
        {
            if(item.name == null)
            {
                break;
            }
            else if (item.name == "GreyKey")
            {
                keyFound = true;
                itemTmp = item;
            }
        ...
        if (needsKey)
        {
            if (Input.GetKeyDown(KeyCode.E) && nextDoor && usedKey && opened) // если персонаж находится в зоне двери то при нажатии кнопки Е мы переходим на следующую сцену и сохранем все о нем данные
            {
    ...
    private void OnTriggerExit(Collider other) // проверка на коллизию с персонажем
    {
