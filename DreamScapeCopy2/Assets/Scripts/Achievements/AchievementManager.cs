...
{
    public AudioSource getAchievementAudio;
    ...
    void Start()
    {
        ...
        CreateAchievement("General", "Press I", "Press I to open inventory", 0);
        CreateAchievement("General", "Dash", "Find out how to dash", 2);
        CreateAchievement("General", "Jump", "Find out how to jump", 3);

        CreateAchievement("Other", "All keys", "Press all keys to unlock", 1, new string[] { "Press W", "Press A", "Press S", "Press D", "Jump", "Dash", "Press I" });
    ...
    {
        if (achievements[title].EarnAchievement()) //проверка если ачивка уже открыта или нет, если нет, то показываем на экране
        {
            getAchievementAudio.Play();
    ...
    public IEnumerator HideAchievement(GameObject achievement) //Показываем на экране 4 секунды потом удаляем
    {
    ...
    public void SetAchievementInfo(string parent, GameObject achievement, string title)//Возвращает всю информацию об ачивке из словаря achievements(размер,какое изображение использовать, чей ребонок и тд.)
    {
